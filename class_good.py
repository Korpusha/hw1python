...
class Goods:
    ...
    def is_eatable(self):
        ...
        """
        if self.category != 'household chemistry':
            return True
        return False
...
class Basket:
    def __init__(self, what_in, amount):
        self.what_in = what_in
        self.amount = amount

    def my_total(self):
        """Same as price_total method, but sums them all together"""
        total = [Goods.price_total(product) * number for product, number in zip(self.what_in, self.amount)]
...
my_basket = Basket([first_good, second_good, third_good, fourth_good, fifth_good], [1, 2, 3, 4, 5])
